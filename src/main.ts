...
program
  ...
  .addOption(
    new Option(
      '--remote <file>',
      'remote repository URL to fetch git info from',
    ),
  )
  .addOption(
    new Option(
  ...
  .addOption(
    new Option(
      '--include <pattern>',
      'include files matching the pattern, ignored if specific files are provided',
    ).default(config.include),
  )
  .addOption(
    new Option(
      '--exclude <pattern>',
      'exclude files matching the pattern, ignored if specific files are provided',
    )
      .conflicts('include')
  ...
  .addOption(
    new Option(
      '-r, --recent [days]',
      'only include the most recently (7 days) modified files per directory',
    ).default(config.recent),
  )
  .addOption(
    new Option(
      '--preview [lines]',
      'enable preview features, if not specified, defaults to 5 lines',
...
async function main(options: Options, args: Array<string>): Promise<void> {
  ...
  const includePatterns = options.include
    ? String(options.include).split(',')
    : ['**/*'];
  const excludePatterns = options.exclude
    ? String(options.exclude).split(',')
    : [];
  ...
  if (args.length === 1 && fs.statSync(currentWorkingDirectory).isDirectory()) {
    filepathArray = await getFilesFromDirectory(
  ...
  if (options.recent) {
    ...
    filepathArray = getRecentModifiedFiles(
      currentWorkingDirectory,
      filepathArray,
      recentDays,
    );
  ...
  const outputString = await getOutputString(
    currentWorkingDirectory,
    gitInfo,
    filepathArray,
    options.preview,
